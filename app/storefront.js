...
export default function Storefront() {
  ...
  const update = (id, delta) => setCart(old => ({ ...old, [id]: Math.max(0, (old[id] || 0) + delta) }));
  ...
  return <main>
    ...
    <section id="beranda" className="hero section-shell">
      <div className="hero-copy">
        ...
        <div className="hero-actions"><a className="btn btn-wa" href="#produk">◉ PESAN SEKARANG</a><a className="btn btn-outline" href={SHOPEE_URL} target="_blank" rel="noopener noreferrer">🛍 BELI DI SHOPEE</a></div>
      </div>
      ...
      <div className="benefit-strip">{benefits.map(([icon, title]) => <div key={title}><i>{icon}</i><span>{title}</span></div>)}</div>
    </section>
    ...
    <section id="produk" className="products-section section-shell">
      ...
      <div className="product-grid">{products.map(product => <article className={`product-card ${product.popular ? 'popular' : ''}`} key={product.id}>{product.popular ? <span className="popular-label">★ PALING LARIS</span> : null}<div className="card-top"><p className="package-name">{product.name}</p><span className="save-chip">HEMAT {rupiah(product.saving)}</span></div><div className="product-thumb"><Image src={product.image} alt={`${product.name} RULOXIUS ${product.size}`} fill sizes="(max-width: 800px) 100vw, 360px" /><span className="product-photo-label"><small>{product.name}</small><b>{product.size}</b></span></div><div className="product-info"><h3>{product.size}</h3><p className="product-detail"><span>✓</span>{product.detail}</p><p className="unit-price">{product.unit}</p><div className="price-line"><strong>{rupiah(product.price)}</strong><del>{rupiah(product.compareAt)}</del></div><div className="stock-line"><span><i/> Stok tersedia</span><span>Siap dikirim</span></div><div className="qty"><button onClick={() => update(product.id,-1)} aria-label={`Kurangi ${product.size}`}>−</button><span>{cart[product.id] || 0}</span><button onClick={() => update(product.id,1)} aria-label={`Tambah ${product.size}`}>+</button></div><button className="choose-package" onClick={() => update(product.id,1)}>{cart[product.id] ? 'TAMBAH LAGI' : 'PILIH PAKET'} <span>→</span></button></div></article>)}</div>
    ...
    <section id="faq" className="faq section-shell"><div className="section-heading"><small>YANG SERING DITANYAKAN</small><h2>FAQ</h2></div>{[['Apakah aman untuk semua keramik?','Gunakan sesuai petunjuk, tes dahulu pada area kecil, dan hindari permukaan sensitif terhadap cairan pembersih.'],['Bagaimana cara pakainya?','Gunakan sarung tangan, oles pada area berkerak, tunggu sebentar, sikat, lalu bilas sampai bersih.'],['Berapa lama pesanan diproses?','Pesanan terverifikasi diproses 1–2 hari kerja sebelum diserahkan ke kurir.']].map(([q,a]) => <details key={q}><summary>{q}<span>+</span></summary><p>{a}</p></details>)}</section>

...
export default function Storefront() {
  ...
  return <main>
    ...
    <section id="produk" className="products-section section-shell">
      ...
      <div className="product-grid">{products.map(product => <article className={`product-card ${product.popular ? 'popular' : ''}`} key={product.id}>{product.popular ? <span className="popular-label">★ PALING LARIS</span> : null}<div className="card-top"><p className="package-name">{product.name}</p><span className="save-chip">HEMAT {rupiah(product.saving)}</span></div><div className="product-thumb"><Image src={product.image} alt={`${product.name} RULOXIUS ${product.size}`} fill sizes="(max-width: 800px) 100vw, 360px" /></div><div className="product-info"><h3>{product.size}</h3><p className="product-detail"><span>✓</span>{product.detail}</p><p className="unit-price">{product.unit}</p><div className="price-line"><strong>{rupiah(product.price)}</strong><del>{rupiah(product.compareAt)}</del></div><div className="stock-line"><span><i/> Stok tersedia</span><span>Siap dikirim</span></div><div className="qty"><button onClick={() => update(product.id,-1)} aria-label={`Kurangi ${product.size}`}>−</button><span>{cart[product.id] || 0}</span><button onClick={() => update(product.id,1)} aria-label={`Tambah ${product.size}`}>+</button></div><button className="choose-package" onClick={() => update(product.id,1)}>{cart[product.id] ? 'TAMBAH LAGI' : 'PILIH PAKET'} <span>→</span></button></div></article>)}</div>
